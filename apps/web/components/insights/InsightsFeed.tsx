        ...
        {filtered.map((insight, idx) => {
          ...
          return (
            <article
              key={insight.id || idx}
              className={clsx(
                'prism-panel-master p-6 sm:p-8 border-l-4 transition-all duration-300 hover:shadow-xl hover-lift bg-gradient-to-br from-white via-white to-slate-50/40',
                config.accentClass
              )}
            >
                ...
                <div className="flex items-center gap-2.5 flex-wrap">
                  ...
                  >
                    {insight.severity === 'critical' && (
                      <span className="h-2 w-2 rounded-full bg-rose-500 animate-ping mr-0.5" />
                    )}
                  ...
                  {insight.dimension && (
                    <span className="font-mono text-xs text-slate-600 px-2.5 py-0.5 rounded-full bg-slate-100 border border-slate-200/80 font-semibold">
                      {insight.dimension}: {insight.segment || 'Geral'}
                    </span>
                  )}
                </div>

                <div className="flex items-center gap-2 font-mono text-xs text-slate-500 bg-slate-50 px-2.5 py-1 rounded-md border border-slate-200/60">
                  <ShieldCheck className="h-3.5 w-3.5 text-emerald-600" />
                  <span>Confiança: <strong className="text-slate-800 font-bold">{((insight.confidence || 0.95) * 100).toFixed(0)}%</strong></span>
                </div>
              </div>
              ...
              <div className="mt-5 p-4 rounded-xl border border-slate-200/80 bg-slate-50/60 space-y-2">
              ...
              {/* Hypothesis */}
              {insight.hypothesis && (
                <div className="mt-3 p-3.5 rounded-xl border border-indigo-200/60 bg-indigo-50/40 text-xs text-slate-700 leading-relaxed">
                  <strong className="text-indigo-900 font-bold font-mono">Hipótese Causal: </strong>
                  {insight.hypothesis}
                </div>
              )}
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3 font-mono text-xs tabular-nums">
                  ...
                  <span
                    className={clsx(
                      'font-bold px-2.5 py-1 rounded-md border shadow-2xs',
                      insight.change > 0 ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'
                    )}
                  >
                ...
                <Link
                  href={`/ask?q=${encodeURIComponent(insight.ask_query || insight.title)}`}
                  className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-indigo-50 hover:bg-indigo-600 border border-indigo-200 hover:border-indigo-600 text-indigo-700 hover:text-white text-xs font-bold transition-all duration-200 shadow-2xs group hover-lift"
                >
                  <Bot className="h-3.5 w-3.5 text-indigo-600 group-hover:text-white transition-colors" />
