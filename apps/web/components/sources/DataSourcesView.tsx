...
export const DataSourcesView: React.FC<DataSourcesViewProps> = ({ initialData }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-5 rounded-xl bg-prism-bg-card border border-prism-border-subtle space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-prism-border-subtle/60">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-prism-accent-blue/10 border border-prism-accent-blue/30 text-prism-accent-blue">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded text-[11px] font-mono bg-emerald-950/60 text-emerald-400 border border-emerald-800/40 font-medium flex items-center gap-1">
          ...
          <div className="flex items-center gap-3 text-xs font-mono">
            <div className="flex items-center gap-1 text-prism-text-muted">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
          <div className="p-3 rounded-lg bg-prism-bg-base/80 border border-prism-border-subtle flex items-center gap-2.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {sources.map((src) => {
            ...
            return (
              <div
                key={src.id}
                className={`p-4 rounded-xl bg-prism-bg-card border transition-all flex flex-col justify-between space-y-3 ${
                  src.is_default
                    ? 'border-prism-accent-blue/50 shadow-sm'
                    : 'border-prism-border-subtle hover:border-prism-border-strong'
                }`}
              >
                <div className="space-y-2">
                  ...
                  <h5 className="text-sm font-semibold text-prism-text-primary">{src.name}</h5>
                  <p className="text-xs text-prism-text-secondary leading-relaxed">
                    {src.description}
                  </p>
                </div>

                <div className="pt-2 border-t border-prism-border-subtle/50 text-[11px] font-mono text-prism-text-muted truncate">
      ...
      <div className="space-y-3">
        ...
        <div className="rounded-xl border border-prism-border-subtle overflow-hidden bg-prism-bg-card">
          <table className="w-full text-left font-mono text-xs">
            <thead>
              <tr className="bg-prism-bg-elevated/80 border-b border-prism-border-subtle text-[10px] text-prism-text-muted uppercase">
                ...
                <th className="py-2.5 px-4 font-semibold">Record Count</th>
                <th className="py-2.5 px-4 font-semibold">Description</th>
                <th className="py-2.5 px-4 font-semibold">Allowlisted Columns</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-prism-border-subtle/40">
              {active_source.available_tables.map((t) => (
                <tr key={t.table_name} className="hover:bg-prism-bg-elevated/30 transition-colors">
                  <td className="py-3 px-4 font-semibold text-prism-accent-blue">{t.table_name}</td>
                  <td className="py-3 px-4 text-prism-text-primary">{t.row_count.toLocaleString()}</td>
                  <td className="py-3 px-4 text-prism-text-secondary max-w-xs">{t.description}</td>
                  <td className="py-3 px-4 text-prism-text-muted text-[10px]">{t.columns.slice(0, 4).join(', ')}... ({t.columns.length} total)</td>
                </tr>
              ))}
            </tbody>
          </table>
