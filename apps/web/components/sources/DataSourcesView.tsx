...
export const DataSourcesView: React.FC<DataSourcesViewProps> = ({ initialData }) => {
  ...
  const [activeTab, setActiveTab] = useState<'topology' | 'catalog'>('topology');
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      {/* Tab Switcher: AWS Topology vs Engine Catalog */}
      <div className="flex items-center gap-2 p-1.5 rounded-2xl bg-white border border-slate-200 shadow-2xs w-fit">
        ...
        >
          ...
          <span>Topologia de Serviços (AWS Architecture)</span>
        </button>
        ...
        >
          ...
          <span>Catálogo de Dados & Governança</span>
        </button>
      </div>

      {/* View 1: Architecture Topology Map */}
      {activeTab === 'topology' && <ServiceTopologyMap />}

      {/* View 2: Engine Catalog & Telemetry */}
      {activeTab === 'catalog' && (
        <div className="space-y-8 animate-fade-in">
