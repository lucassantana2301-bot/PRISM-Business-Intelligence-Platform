...
dimensional breakdowns, time-grain aggregation, and comparison variance analysis.
"""
...
class AnalyticsEngine:
    ...
    def _init_tables(self):
        """Register CSV data sources as typed DuckDB views with standardized column aliases."""
    ...
    def _build_filter_sql(self, filters: List[AnalyticsFilter], table_alias: str = "") -> str:
        """Construct parameterized SQL WHERE clause from AnalyticsFilter list."""
        if not filters:
            return ""
        ...
        for f in filters:
            ...
            if op == FilterOperator.EQ:
                if isinstance(val, str):
                    ...
                    clauses.append(f"{col} = {str(val).upper()}")
                else:
                    clauses.append(f"{col} = {val}")
                ...
                if isinstance(val, list) and len(val) == 2:
                    clauses.append(f"{col} BETWEEN '{val[0]}' AND '{val[1]}'")
        ...
        if time_grain or dimensions:
            ...
            if time_grain:
                if time_grain == TimeGrain.DAY:
                    grain_expr = "CAST(DATE_TRUNC('day', o.created_at) AS VARCHAR)"
                elif time_grain == TimeGrain.WEEK:
                    grain_expr = "CAST(DATE_TRUNC('week', o.created_at) AS VARCHAR)"
                elif time_grain == TimeGrain.MONTH:
                    grain_expr = "CAST(DATE_TRUNC('month', o.created_at) AS VARCHAR)"
                elif time_grain == TimeGrain.QUARTER:
                    grain_expr = "CAST(DATE_TRUNC('quarter', o.created_at) AS VARCHAR)"
                elif time_grain == TimeGrain.YEAR:
                    grain_expr = "CAST(DATE_TRUNC('year', o.created_at) AS VARCHAR)"
            ...
            if query.order_by:
                custom_orders = [f"{o.field} {o.direction.upper()}" for o in query.order_by]
                compiled_sql += f" ORDER BY {', '.join(custom_orders)}"
            elif order_by_cols:
                compiled_sql += f" ORDER BY {', '.join(order_by_cols)}"

            if query.limit:
                compiled_sql += f" LIMIT {query.limit}"
            if query.offset:
                compiled_sql += f" OFFSET {query.offset}"
